...
const ALLOWED_ORIGINS = [
  "https://tru-sentry.com",
  "https://www.tru-sentry.com",
];
  ...
  try {
    ...
    if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_FROM && OWNER_PHONE) {
      ...
      await client.messages.create({
        from: TWILIO_FROM, // your Twilio number
        to: OWNER_PHONE,   // for trial: must be a Verified Caller ID
        body: `New TruSentry quote:
Name: ${name}
Email: ${email}
Phone: ${phone || "N/A"}
...
${source ? `Source: ${source}` : ""}`
  ...
  return res.status(200).json({ ok: true, sms: smsSent, smsError });
}
